...
BayesBase.mean_cov(sl::SampleList) = sample_list_mean_cov(sl, Val(true))
BayesBase.mean_var(sl::SampleList) = sample_list_mean_var(variate_form(typeof(sl)), sl)
...
function BayesBase.mean_precision(sl::SampleList)
    μ, Σ = mean_cov(sl)
    return μ, inv(Σ)
end

function BayesBase.weightedmean_precision(sl::SampleList)
...
function BayesBase.rand(rng::AbstractRNG, samplelist::SampleList, n::Integer)
...
function approximate_prod_with_sample_list(
    ...
    @assert variate_form(typeof(x)) === variate_form(typeof(y)) "Unsupported SampleList prod approximation: variate forms should match"
